#!/usr/bin/env python3
"""
Unified Model Training Framework for SMS Spam Classification
...
class UniversalModelTrainer:
    ...
    def save_model(self) -> None:
        ...
        logger.info(f"Model and vectorizer saved to {self.results_dir}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Universal Model Trainer for SMS Spam Classification')
    parser.add_argument('--algorithm', required=True, 
                       help='Algorithm to train (naive_bayes, logistic_regression, random_forest, svm, knn, adaboost, decision_tree, gradient_boosting)')
    ...
    try:
        ...
        if 'test' in results:
            ...
            print(f"Test ROC-AUC: {roc_auc:.4f}" if isinstance(roc_auc, (int, float)) else f"Test ROC-AUC: {roc_auc}")
