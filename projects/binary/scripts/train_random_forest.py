#!/usr/bin/env python3
"""
Random Forest Training Script for SMS Spam Binary Classification

This script trains a Random Forest model on the SMS spam dataset
...
class SMSRandomForestTrainer:
    """Trainer for SMS spam classification using Random Forest."""
    ...
    def train_model(self, n_estimators=100, max_depth=None, min_samples_split=2, 
        ...
        self.model = RandomForestSklearn(
    ...
    def hyperparameter_tuning(self):
        ...
        # Create base model
        base_model = RandomForestSklearn(random_state=42, n_jobs=-1)
    ...
    def plot_confusion_matrix(self, cm, title="Confusion Matrix"):
        ...
        # Save plot
        plot_path = self.results_dir / f"random_forest_{title.lower().replace(' ', '_')}.png"
    ...
    def plot_feature_importance(self, top_n=20):
        ...
        # Save plot
        plot_path = self.results_dir / "random_forest_feature_importance.png"
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def plot_training_curves(self):
        ...
        if not hasattr(self.model.model, 'estimators_'):
            print("Model doesn't have individual estimators for plotting")
            return
        ...
        for i, estimator in enumerate(self.model.model.estimators_):
            train_score = estimator.score(self.X_train.toarray(), self.y_train)
            val_score = estimator.score(self.X_val.toarray(), self.y_val)
            train_scores.append(train_score)
            val_scores.append(val_score)
            
        plt.figure(figsize=(10, 6))
        plt.plot(range(1, len(train_scores) + 1), train_scores, label='Training Score')
        plt.plot(range(1, len(val_scores) + 1), val_scores, label='Validation Score')
        ...
        # Save plot
        plot_path = self.results_dir / "random_forest_learning_curve.png"
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def save_model(self):
        ...
        # Save model
        model_path = self.results_dir / "random_forest_model.pkl"
        with open(model_path, 'wb') as f:
            pickle.dump(self.model, f)
            
        # Save vectorizer
        vectorizer_path = self.results_dir / "random_forest_vectorizer.pkl"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train Random Forest model for SMS spam classification')
    ...
    # Initialize trainer
    trainer = SMSRandomForestTrainer(args.data_dir, args.results_dir)
