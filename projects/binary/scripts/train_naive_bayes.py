#!/usr/bin/env python3
"""
Naive Bayes Training Script for SMS Spam Binary Classification

This script trains a Multinomial Naive Bayes model on the SMS spam dataset
...
class SMSNaiveBayesTrainer:
    """Trainer for SMS spam classification using Naive Bayes."""
    ...
    def prepare_features(self, vectorizer_type='count', max_features=None, ngram_range=(1, 1)):
        """
    ...
    def train_model(self, alpha=1.0, variant='multinomial'):
        """
        Train the Naive Bayes model.
        
        Args:
            alpha: Smoothing parameter
            variant: Type of Naive Bayes ('multinomial', 'bernoulli')
        """
        print(f"Training Naive Bayes model (variant={variant}, alpha={alpha})...")
        
        self.model = NaiveBayesSklearn(
            variant=variant,
            alpha=alpha,
            random_state=42
        )
    ...
    def plot_confusion_matrix(self, cm, title="Confusion Matrix"):
        ...
        # Save plot
        plot_path = self.results_dir / f"naive_bayes_{title.lower().replace(' ', '_')}.png"
    ...
    def save_model(self):
        ...
        # Save model
        model_path = self.results_dir / "naive_bayes_model.pkl"
        with open(model_path, 'wb') as f:
            pickle.dump(self.model, f)
            
        # Save vectorizer
        vectorizer_path = self.results_dir / "naive_bayes_vectorizer.pkl"
    ...
    def run_training_pipeline(self, vectorizer_type='count', alpha=1.0, variant='multinomial'):
        """Run the complete training pipeline."""
        print("="*60)
        print("NAIVE BAYES TRAINING PIPELINE")
        print("="*60)
        ...
        # Train model
        self.train_model(alpha=alpha, variant=variant)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train Naive Bayes model for SMS spam classification')
    parser.add_argument('--data-dir', type=str, help='Path to data directory')
    parser.add_argument('--results-dir', type=str, help='Path to results directory')
    parser.add_argument('--vectorizer', type=str, default='count', choices=['count', 'tfidf'],
    ...
    parser.add_argument('--variant', type=str, default='multinomial', 
                       choices=['multinomial', 'bernoulli'], help='Naive Bayes variant')
    
    args = parser.parse_args()
    
    # Initialize trainer
    trainer = SMSNaiveBayesTrainer(args.data_dir, args.results_dir)
